...
业务实现为 `app.content.review.similarity.SimilarityService`，MCP 服务入口为 `python -m app.content.mcp_server`。工具实际执行 Python 和 Qdrant 检索，不能由语言模型估算相似度。详细配置、示例及恢复规则见项目 `docs/同质化检测-交付与验收.md`。
