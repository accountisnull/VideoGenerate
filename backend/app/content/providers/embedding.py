...
class LocalEmbedding:
    ...
    async def aembed(self, script: str) -> list[float]:
        ...
        try:
            ...
            logger.error("模型子进程启动失败 reason=PROCESS_START")
            raise SimilarityError("EMBEDDING_FAILED", "无法启动模型进程") from None
        try:
            async with asyncio.timeout(self.timeout_seconds):
                output, _diagnostics = await process.communicate(
            ...
            if isinstance(error, asyncio.CancelledError):
                raise
            raise SimilarityError("EMBEDDING_TIMEOUT", "模型加载或推理超时") from None
        return self._decode(output.decode("utf-8"), process.returncode)

    def _decode(self, output: str, returncode: int) -> list[float]:
        if returncode:
            logger.error("模型子进程失败 reason=PROCESS_EXIT exit_code=%s", returncode)
            raise SimilarityError("EMBEDDING_FAILED", "模型进程异常退出")
        try:
            result = json.loads(output)
            if "error" in result:
                details = result["error"]
                category = result.get("diagnostic", "MODEL_ERROR")
                # 仅允许安全标识，不记录第三方原始 stderr、正文或路径。
                if not isinstance(category, str) or not category.isidentifier():
                    category = "MODEL_ERROR"
                logger.error("模型推理失败 reason=%s", category)
    ...
    def embed(self, script: str) -> list[float]:
        ...
        return self._decode(process.stdout, process.returncode)
