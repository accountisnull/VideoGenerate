...
当前分支已实现 `/api/health`、`/api/checks` 等环境自检接口，以及独立内容服务 `/v1/content` 下的 health、capabilities、作业提交和查询。内容服务已接入持久化与后台编排，但默认尚未组装真实模型，不能据此宣称生成或发布已接通。部署与限制见 [内容服务](content-service.md)。领域用语见 [CONTEXT.md](../CONTEXT.md)，职责依据见 [技术框架](技术框架.md)。

公共 Pydantic 数据模型与可分发的 JSON Schema 已实现，使用方法和校验边界见 [公共数据契约](contracts/README.md)。内容服务已复用公共模型并实现作业幂等与状态持久化；配音、数字人、媒体文件校验和真实发布仍需后续接入。本次不改变 v1 字段。
