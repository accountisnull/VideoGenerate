...
严格审核输出、数据库 v2 升级、备份与样例实测见 [内容审核与存储](content-audit-storage.md)。
...
数据库使用专用 application_id 和 user_version=2。空库启动时在事务内建表；v1 库无损升级至 v2，已有同版本库复用，其他应用库或未知版本拒绝启动，不删除原数据。迁移定义位于 app/content/storage/migrations.py，升级增加调用查询索引、终态和已完成调用的更新保护。迁移失败整体回滚，备份及回退方法见 [内容审核与存储](content-audit-storage.md)。
